...
1. Create the first Lambda function (lambda_mock_data_generator.py) to generate mock data, deploy the code, add test event to test the code and test the code.
...
4. Create the second lambda function (lambda_data_to_s3.py) to insert data into S3 bucket, provide relevant IAM Role to connect S3 and lambda.
...
9. Create a Glue Job, enable job bookmarking and write pyspark script (glue_data_ingestion_to_dynamo.py) to read data incrementally.
